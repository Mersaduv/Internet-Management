...
function Build-IosFromCi([string]$Version, [string]$Sha) {
  Write-Step "Wait for GitHub Actions IPA (sha $($Sha.Substring(0,7)))"
  ...
  while ((Get-Date) -lt $deadline) {
    $runs = Invoke-JsonGet "https://api.github.com/repos/$Repo/actions/workflows/ios-ipa.yml/runs?per_page=10"
    ...
    Write-Host "  run $runId — status=$status conclusion=$conclusion" -ForegroundColor DarkGray
  ...
  if (-not $runId) {
    throw "No CI run found for $Sha within $TimeoutMinutes minutes."
  }
  if ($conclusion -ne 'success') {
    throw "CI run $runId finished with conclusion=$conclusion. See https://github.com/$Repo/actions/runs/$runId"
  }

  $tag = "ios-ipa-$Version"
  $encodedTag = [uri]::EscapeDataString($tag)
  $release = Invoke-JsonGet "https://api.github.com/repos/$Repo/releases/tags/$encodedTag"
  $asset = $release.assets | Where-Object { $_.name -like 'Jahan_Bit-*.ipa' } | Select-Object -First 1
  if (-not $asset) {
    throw "IPA asset not found on release $tag"
  }

  $dist = Join-Path $Root 'dist\ios'
  New-Item -ItemType Directory -Force -Path $dist | Out-Null
  # Keep only current IPA name
  Get-ChildItem -LiteralPath $dist -Filter '*.ipa' -ErrorAction SilentlyContinue |
    Remove-Item -Force -ErrorAction SilentlyContinue

  $dest = Join-Path $dist $asset.name
  Write-Step "Download $($asset.name)"
  ...
  Write-Host "CI: https://github.com/$Repo/actions/runs/$runId" -ForegroundColor DarkGray
}
...
if ($PSVersionTable.PSEdition -eq 'Core' -and $IsMacOS) {
  ...
  try { if ((& uname -s 2>$null) -eq 'Darwin') { $isMac = $true } } catch { }
}

Write-Step "iOS IPA — v$version"
